...
func (doc *NewCreatedDoc) AddParagraph(text string) *document.Paragraph {
	// 添加段落/文本
	return doc.doc.AddParagraph(text)
}
func (doc *NewCreatedDoc) GetStyleManager() *style.StyleManager {
	// 获取样式管理器
	return doc.doc.GetStyleManager()
...
// SaveWord 保存文档到指定路径
func (doc *NewCreatedDoc) SaveWord() {

	outputFile := fmt.Sprintf("output/%s/generated.docx", time.Now().Format("20060102"))
